...
#my $sourcetype = "d";
#my $source = "bhl_paragraphs"; #source table
#my $db = "paragraphbootstrappingevaluation";
#my $mode = "plain";
#my $seedfile ="Z:\\seeds";
#my $prefix = "bhl_05_10_05_20_05";
#my $benchmark = "bhl_benchmark";
#my $mothertable ="bhl_paragraphs";

#evaluation collection: plazi ant
...
my $sourcetype = "d";
my $source = "plaziants_paragraphs"; #source table
my $db = "paragraphbootstrappingevaluation";
my $mode = "plain";
my $seedfile ="Z:\\seeds";
my $prefix = "plaziants_05_10_05_20_05";
my $benchmark = "plaziants_benchmark";
my $mothertable ="plaziants_paragraphs";

#evaluation collection: plazi fish
#my $sourcetype = "d";
#my $source = "plazifish_paragraphs"; #source table
#my $db = "paragraphbootstrappingevaluation";
#my $mode = "plain";
#my $seedfile ="Z:\\seeds";
#my $prefix = "plazifish_05_10_05_20_05";
#my $benchmark = "plazifish_benchmark";
#my $mothertable ="plazifish_paragraphs";
...
@organ_names = filterLearnedOrganNames(@organ_names);
foreach (@organ_names){
...
#if a "namesunique" table exists in the database: filter learned organ names against the taxon names
#also filter learned organ names against headings/subheadings which are identified by the pattern of a word/string starting a paragraph that followed by a capitalized word, for example "biology Unknow"  
sub filterLearnedOrganNames{
	my @organnames = @_;
	my $nametable;
	my @filtered;
	# if namesunique table exists
	my $select = $dbh->prepare('show tables');
	$select->execute() or warn "$select->errstr\n";
	while(my ($table) = $select->fetchrow_array()){
		if ($table =~/namesunique/){
			$nametable = $table;
			last;
		}
	}
	#filter against taxon names
	foreach my $name (@organnames){
		if(!istaxonname($name, $nametable) and !isheading($name)){
			push(@filtered, $name)
		}
	}
	return @filtered;
...
#check to see if the name is a heading or a Named Entities
sub isheading{
	my $name = shift;
	my $result = 0;
	if($name!~/\w/){
	...
	$name =~ s/([a-z])(\w+)/\u$1$2/;;
	my $name1 = $name; #capitalized
	$name =~ tr/a-z/A-Z/;
	my $name2 = $name;#all-cap
	$select = $dbh->prepare('select count(*) from '.$source.' where paragraph COLLATE utf8_bin rlike "[a-zA-Z0-9]+ '.$name2.' "');
	
	#$select = $dbh->prepare('select count(*) from '.$source.' where paragraph COLLATE utf8_bin rlike "[a-zA-Z0-9]+ '.$name1.' " or paragraph COLLATE utf8_bin rlike "[a-zA-Z0-9]+ '.$name2.' "');
	$select->execute() or warn "$select->errstr\n";
