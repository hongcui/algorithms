...
#perl ..\paragraphExtraction\bootstrapDescriptionExtraction.pl d bhl_paragraphs markedupdatasets plain Z:\DATA\BHL\target\seeds bhl_clean
...
print "\tsourcetype: either f (file system) or d (database)\n";
...
if(@seeds == 0){
@seeds = SeedDescriptionExtraction::collectSeedParagraphs($db, "clean_paragraphs", $prefix);
}
foreach (@seeds){
