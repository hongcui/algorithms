...
public class TaxonXExtractor {
	...
	/**
	 * @author hongcui split taxonX documents to paragraphs for description
	 *         paragraph extraction evaluation. output a benchmark table and a
	 *         paragraphs table
	 * 
	 */

	private File source = new File(
			"Z:\\DATA\\Plazi\\2ndFetchFromPlazi\\taxonX-ants");
	...
	public TaxonXExtractor() {
		try {
			...
			conn = DriverManager
					.getConnection("jdbc:mysql://localhost/sourcedatasets?user=root&password=root");
			Statement stmt = conn.createStatement();
			stmt.execute("drop table if exists " + benchmark);
			stmt.execute("create table if not exists "
					+ benchmark
					+ " (paraID varchar(100) NOT NULL, paragraph text, isDescription varchar(5), primary key(paraID))");

			stmt.execute("drop table if exists " + paragraphs);
			stmt.execute("create table if not exists "
					+ paragraphs
					+ " (paraID varchar(100) NOT NULL, paragraph text, remark text, flag varchar(20), primary key(paraID))");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void createEvaluationTables() {
		...
		try {
			...
			for (int f = 0; f < files.length; f++) {
				...
				List<Element> treatments = XPath.selectNodes(root,
						"/tax:taxonx/tax:taxonxBody/tax:treatment");
	...
	private void extractFromTreatment(String fn, int t, Element treatment) {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			if (e.getName().compareTo("nomenclature") != 0) { // div, ref_group
				...
				String isDescription = type.compareTo("description") == 0
						|| type.compareTo("key") == 0 ? "yes" : "no";
				count = extractParagraphs(fn, t, count, e, isDescription); // <tax:p>
																			// in
																			// element
			} else {
				sb = new StringBuffer();
	...
	private int extractParagraphs(String fn, int t, int count, Element e,
	...
	private void insert2Tables(String fn, int t, int count, String text,
			String isDescription) {
		try {
			...
			stmt.execute("insert into " + benchmark + " values(\"" + paraID
					+ "\",\"" + text + "\",\"" + isDescription + "\")");
			stmt.execute("insert into " + paragraphs + " values(\"" + paraID
					+ "\",\"" + text + "\",\"\",\"\")");
		} catch (Exception e) {
			e.printStackTrace();
		}
